  ...
  return (
    <div className={`gap-y-[8px] flex flex-col ${classes}`}>
      {label && (
        <label
          htmlFor={id}
          className=" !text-[16px] uppercase block font-medium text-secondary-alt font-haasBold"
        >
          {label}
        </label>
      )}
...
const Login = ({ classes, close, isLightbox = true, data = {} }) => {
  ...
  const logoAlt =
    (logo && typeof logo === "object" && logo.alt) || "Hensley Logo";
  ...
  return (
    <div
      className={`${classes} lg:bg-transparent bg-[#F4F1EC] w-full flex justify-center items-center z-[${isLightbox ? 100 : 0}] relative lg:w-[600px] lg:py-[20px]  mx-auto`}
    >
      <form
      ...
      >
        ...
        <div className="w-full grid grid-cols-2">
          ...
          <label className="block text-[16px] leading-[19px] font-haasLight text-secondary-alt mb-2 text-right">
            <button
              ...
              className={
                "underline text-secondary-alt font-haasRegular hover:opacity-70 transition-opacity duration-200"
              }
            >
              {forgetPasswordLabel}
            </button>
        ...
        >
          <span
            className={`font-haasLight uppercase text-[16px] hover:border-secondary-alt
          ...
          {agreementContent ? (
            <div className="w-full sm:max-w-[370px] ">
              {convertToHTMLRichContent({
                ...
                class_p:
                  "text-[12px] leading-[16px] text-secondary-alt font-haasRegular uppercase text-center",
                class_a:
                  "text-[12px] text-secondary underline hover:opacity-70 transition-opacity duration-200",
              })}
            </div>
          ) : (
            <span className="w-full text-[12px] leading-[16px] text-secondary-alt font-haasRegular uppercase text-center block">
              ...
              <a
                href="/terms-of-use"
                className="text-secondary underline hover:opacity-70 transition-opacity duration-200"
              >
        ...
        {/* New User Section */}
        <span className="font-recklessRegular text-[27px] leading-[20px] text-secondary-alt block lg:mt-0 mt-[100px]">
          {newToHensleyText}
        </span>
