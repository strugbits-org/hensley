...
export const TestimonialCard = ({ data, classes, imageExp,titleClass }) => {
    const { name, title, feedback, description, image } = data;
    return (
        <div className={`flex flex-col lg:flex-row group  bg-primary lg:bg-transparent relative min-h-[499px] lg:h-full w-full lg:border border-primary-border duration-300 ease-in-out max-w-[1240px] flex-shrink-0 ${classes}`}>
            ...
            <div className='w-full lg:w-1/2 p-6 flex flex-col'>
                ...
                <p className="text-center lg:text-start my-[16px] text-[14px] lg:text-[18px] leading-[30px] text-secondary-alt font-haasRegular uppercase">{feedback || description}</p>
                <span className={`mt-auto text-center lg:text-start text-[12px] leading-[42px] lg:text-[12px] lg:leading-[42px] uppercase tracking-wider text-secondary-alt font-recklessRegular ${titleClass}`}>{title}</span>
            </div>
        </div>
    )
}