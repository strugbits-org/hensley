...
export const Header = ({ data = {}, marketsData = [], tentsData = [] }) => {
  ...
  useEffect(() => {
    const quantity =
      cookies?.cartQuantity !== undefined ? String(cookies.cartQuantity) : "0";
    setCartTotalQuantity(quantity);
  }, [cookies.cartQuantity]);

  const getMenuUrl = (item) => item?.slug || item?.href || item?.url || "";

  const isMarketsMenu = (item) =>
    item?.type === "markets" ||
    String(item?.title || "").toUpperCase() === "MARKETS";
  const isTentsMenu = (item) =>
    item?.type === "tents" ||
    /(^|\b)tents?(\b|$)/i.test(String(item?.title || ""));
  ...
  const getSubItemsForMenu = (menuItem) => {
    ...
    return sortByOrderNumber(
      headerSubMenu.filter((item) =>
        item.Header_menuItems?.some((subItem) => subItem._id === menuItem?._id),
      ),
    );
  };
  ...
  const getNestedItemsForSubMenu = (item) => {
    ...
    return sortByOrderNumber(
      headerMegaMenu.filter((x) =>
        x.HeaderSubMenu_categories?.some((y) => y._id === item?._id),
      ),
    );
  };
  ...
  const handleSubMenuClick = (item, isMobile = false) => {
    ...
    const nestedData =
      itemIsTents ? sortByOrderNumber(tentsData)
      : itemIsMarkets ? sortByOrderNumber(marketsData)
      : getNestedItemsForSubMenu(item);

    const shouldOpenNestedMenu =
      item?.type === "submenu" ||
      itemIsTents ||
      itemIsMarkets ||
      nestedData.length > 0;
  ...
  useEffect(() => {
    const currentMenu =
      header.find((item) => item.title === activeMenu) || header[0];
  ...
  useEffect(() => {
    ...
    const matchedMenu =
      matchedFromMenu ||
      Object.keys(activeMapping).find((menu) =>
    ...
    <>
      <header>
        <div className="hidden lg:block fixed inset-x-0 top-0 z-[110] desktop-menu">
          ...
          <div className="h-[45px] relative">
            ...
            >
              {subNavigation.map((item) => {
                const { title } = item;
                const isActive =
                  activeSubMenuItem?.title === title &&
                  item.type !== "lightbox";

                return (
                  <li
                    key={title}
                    className="h-full flex justify-center items-center relative group"
                  >
                    <button
                      key={title}
          ...
          : (
            selectedMenu &&
            (selectedMenu.type === "markets" || selectedMenu.type === "tents")
          ) ?
            <MarketTentModal
              selectedMenu={selectedMenu}
              closeModal={() => setSelectedMenu(false)}
        ...
        <div className="relative z-[110]">
          <div className="mobile-menu lg:hidden fixed inset-x-3 top-3 z-[110] px-6 py-2">
            <div
              className={`absolute inset-0 -z-10 backdrop-blur-[20px] brightness-[50px] ${isMobileMenuOpen ? "bg-glass-white" : "bg-secondary-glass"}`}
            ></div>
