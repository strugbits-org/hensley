...
export const Footer = ({ data }) => {
  ...
  const brandHeading =
    footerData?.copyrightText1 ||
    footer?.brand?.title ||
    footer?.brand?.eyebrow ||
    "";
  const brandCopy =
    footerData?.copyrightText2 || footer?.bottomBar?.tagline || "";
  const logoUrl =
    ...
    if (item?.icon) {
      return (
        <PrimaryImage
          ...
          fit="fit"
          url={item.icon}
          defaultDimensions={{ height: "24px", width: "24px" }}
          alt={item?.label || item?.title || "social icon"}
        />
      );
    }

    return (
      <span className="text-primary text-sm font-haasMedium uppercase">
        {(item?.platform || item?.title || "").slice(0, 1)}
      </span>
    );
  };
  ...
  return (
    <footer className="relative footer bg-secondary-alt min-h-screen flex flex-col justify-between p-6 pt-12 3xl:p-16 3xl:pt-24 z-[110]">
