  ...
  const { featuredProject, filteredProjects } = useMemo(() => {
    ...
    const filtered = projects.filter((project) => {
      return selectedTags.some(
        (tag) =>
          project.portfolioCategories?.some?.(
            (category) => (category._id || category.id || category) === tag,
          ) ||
          project.markets?.some?.(
            (market) => (market._id || market.id || market) === tag,
          ) ||
          project.studios?.some?.(
            (studio) => (studio._id || studio.id || studio) === tag,
          ),
      );
    });

    // When filtering, show ALL results in the grid — don't split
    // the first one into featuredProject (which hides it from the grid)
    return {
