...
export const FeaturedProjects = ({
  ...
  const featuredProjectTitle =
    pageDetails?.featuredProjectTitle || "Featured Projects";
  ...
  return (
    <div className={`w-full mt-5 md:mt-0 py-20 bg-primary lg:py-6 ${classes}`}>
      ...
      <div
        ref={sliderRef}
        className={`mt-[21px] relative keen-slider h-screen ${isSliderReady ? "opacity-100 visible" : "opacity-0 invisible max-h-[20vh]"}`}
      >
          ...
          return (
            <div key={index} className="keen-slider__slide relative group">
              <PrimaryImage
                useNextImage={true}
                q={60}
                url={portfolioRef.coverImage.imageInfo}
                size="tablet"
                type={"alternate"}
                customClasses="size-full object-cover"
              />
              <CustomLink
