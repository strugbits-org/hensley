...
export const MarketTentModal = ({ selectedMenu, closeModal }) => {
    ...
    return (
        <div onMouseLeave={closeModal} className="relative lg:p-6 lg:h-[calc(100dvh-90px)] flex items-center justify-center lg:overflow-hidden mt-4 lg:mt-0">
                    ...
                    return (
                        <CustomLink key={title} to={`/${type === "markets" ? "market" : "tent"}${slug}`} className={"group relative min-h-[480px] lg:min-h-0 lg:h-full w-full lg:w-1/3 flex justify-center lg:border border-primary-border"} onClick={closeModal}>
                            ...
                            <div className="z-10 py-12 lg:py-6 xl:py-10 px-6 lg:px-8 xl:px-10 2xl:px-12 flex flex-col h-full w-full overflow-hidden">
                                ...
                                <h2 className="lg:mt-4 text-[40px] xs:text-[45px] leading-tight lg:text-[28px] xl:text-[36px] 2xl:text-[48px] uppercase tracking-wider text-primary-alt font-recklessRegular mb-2">
                                    {title}
                                </h2>
