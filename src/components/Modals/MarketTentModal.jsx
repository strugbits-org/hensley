    ...
    return (
        <div onMouseLeave={closeModal} className="relative lg:p-6 lg:h-screen lg:overflow-auto lg:hide-scrollbar mt-12 lg:mt-0">
            ...
            <div className={"w-full lg:h-[54vh] 2xl:h-[86vh] flex justify-center gap-y-3 lg:gap-y-16 lg:gap-6 lg:flex-row flex-col"}>
                {data.map((category) => {
                    const { title, description, tagline, slug, headerCoverImage } = category;
                    ...
                    return (
                        <CustomLink key={title} to={`/${type === "markets" ? "market" : "tent"}${slug}`} className={"group relative min-h-[34rem] lg:h-full w-full lg:w-1/3 flex justify-center lg:border border-primary-border"}>
                            <div className="absolute font-semibold inset-0 lg:inset-6 group-hover:inset-0 transition-[inset] duration-300 ease-in-out">
                                {headerCoverImage && <PrimaryImage timeout={0} url={headerCoverImage} alt={title} customClasses="w-full h-full object-cover brightness-[90%]" />}
                            </div>
                            <div className="z-10 py-[41px] lg:py-[47px] px-[25px] lg:px-[78px] flex flex-col">
                                <h2 className="lg:mt-[26px] text-[45px] lg:text-[38px] 2xl:text-[64px] uppercase tracking-wider text-primary-alt font-recklessLight">{title}</h2>
                                {description || tagline && (
                                    <>
