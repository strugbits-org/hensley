...
export const AddToQuoteButton = ({ text, classes, onClick, disabled }) => {
    return (
        <button onClick={onClick} className={`select-none lg:w-full lg:h-[150px] lg:mt-[110px] sm:mt-6 mt-4 w-full h-[50px] sm:h-[60px] sm:my-6 bg-primary tracking-[6px] group hover:tracking-[10px] transform transition-all duration-300 hover:bg-secondary-alt hover:text-primary relative ${disabled ? 'pointer-events-none opacity-50' : ''} ${classes}`} disabled={disabled}>
            <span className='font-haasLight uppercase lg:text-[14px] leading-[30px] group-hover:font-haasBold'>{text}</span>
            <svg xmlns="http://www.w3.org/2000/svg" width="19.877" height="19.67" viewBox="0 0 19.877 19.67" className='block ml-2 transition-all duration-300 stroke-secondary-alt group-hover:stroke-primary absolute right-[5%] top-1/2 -translate-y-1/2'>
