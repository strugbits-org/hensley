...
export const AddToQuoteButton = ({ text, classes, onClick, disabled }) => {
    return (
        <button onClick={onClick} className={`flex items-center justify-center select-none lg:w-full lg:h-[150px] lg:mt-[110px] sm:mt-6 mt-4 w-full h-[50px] sm:h-[60px] sm:my-6 bg-primary tracking-[2px] sm:tracking-[6px] group sm:hover:tracking-[10px] transform transition-all duration-300 hover:bg-secondary-alt hover:text-primary relative px-[16px] sm:px-0 ${disabled ? 'pointer-events-none opacity-50' : ''} ${classes}`} disabled={disabled}>
