...
import { updateBlogData } from '@/services/admin';
...
const BlogsListUpdate = ({
    ...
    const { author, blogRef, studios = [], markets = [], storeProducts = [] } = data;
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setFilteredBlogs(prev => prev.map(item => item._id === data._id ? { ...item, ...payloadData } : item));
            await updateBlogData(payloadData);
            handleSelectedBlog(null);
        } catch (error) {
            setFilteredBlogs(prev => prev.map(item => item._id === data._id ? data : item));
        ...
        <div className='w-full flex flex-col justify-center items-center text-center py-[50px] gap-y-[40px] relative'>
            ...
            <div className='w-full grid gap-x-[10px] gap-y-[20px] grid-cols-3 mt-[30px]'>
                <div className='flex flex-col gap-y-[5px] col-span-2'>
                    <span className='text-[16px] text-secondary-alt uppercase font-haasBold'>blog title</span>
                    <span className='text-[16px] text-secondary-alt uppercase font-recklessRegular'>{blogRef?.title}</span>
                </div>
                <div className='flex flex-col gap-y-[5px] col-span-3'>
                    <span className='text-[16px] text-secondary-alt uppercase font-haasBold'>Author Name</span>
                    <span className='text-[16px] text-secondary-alt uppercase font-recklessRegular'>{authorName}</span>
                </div>
...
export default BlogsListUpdate;