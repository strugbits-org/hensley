...
const Cards = ({ classes }) => {
    return (
        <div className={`${classes} w-full border text-left border-primary-border flex flex-col gap-y-[10px] p-[10px] `}>
            <div className='flex justify-between'>
                ...
                <div className='flex items-center justify-center rounded-full w-[25px] h-[25px] border border-primary-border transform transition-all duration-300 group cursor-pointer hover:bg-black'>
            ...
            <div className='flex gap-x-[10px] items-center'>
                ...
                <input
                    type="number"
                    className="w-[60px] h-[30px] p-2 bg-transparent border border-primary-border outline-none"
                />
...
const ProductListUpdate = ({ toggle, data }) => {
  const product = data[0]; // showing first product for now
  return (
    <div className='w-full flex flex-col justify-center items-center text-center py-[50px] gap-y-[40px] relative'>
      ...
      <div className='w-full lg:max-w-[500px] flex gap-y-[10px] gap-x-[20px] py-[15px] px-[15px] cursor-pointer border border-primary-border hover:bg-primary transform transition-all duration-30'>
        <div className=' bg-white w-[100px] h-[90px] '>
          <Image src={product.image} className='h-full w-full object-contain' width={100} height={90} />
        </div>
        <div className='w-full text-left flex flex-col gap-y-[10px] justify-center'>
          <span className='font-haasRegular text-secondary-alt uppercase text-[20px] block'>{product.title}</span>
        </div>
      </div>
      ...
      <div className='w-full grid sm:grid-cols-2 grid-cols-1 gap-[20px] items-center justify-center'>
        <Cards />
        <Cards />
        <div className='w-full sm:col-span-2 flex flex-col justify-center items-center'>
          <Cards classes={"w-[440px]"} />
        </div>
      </div>
