...
const Card = ({ data, handleSelectedProject }) => {
    const { portfolioRef, studios, markets } = data;
...
const ProjectsList = ({ data = [], handleSelectedProject, handleSearch }) => {
    ...
    return (
        <div className='w-full flex flex-col justify-center items-center text-center py-[50px] gap-y-[40px]'>
            ...
            <div className='w-full flex flex-col justify-center items-center'>
                ...
                <InputField id="search" placeholder="SEARCH PROJECTS" onChange={handleSearch} borderColor="secondary-alt" classes={"self-center w-[280px]"} />
            </div>
            <div className='w-full flex flex-col gap-y-[30px]'>
                {data.map((item) => (
                    <Card key={item._id} data={item} handleSelectedProject={() => handleSelectedProject(item)} />
                ))}
                {data.length === 0 && <p className='text-secondary-alt text-[20px] font-haasRegular uppercase'>No projects found</p>}
            </div>
        </div>
    );
};
