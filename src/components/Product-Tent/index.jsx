...
const ProductTent = ({
  ...
  const sliderProduct = {
    ...
    mediaItems: tent?.mainMedia
      ? [{ id: "main-media", src: tent.mainMedia, alt: tent?.name || "" }]
      : [],
  };
  ...
  return (
    <>
      ...
      <div className="w-full min-h-screen bg-secondary-alt pt-[75px] 3xl:pt-[120px] px-[24px] 3xl:px-[40px] mt-10 sm:mt-0">
        ...
        <div className="w-full grid gap-[24px] 3xl:gap-[40px] mt-6 3xl:mt-12 lg:grid-cols-[2fr_1fr] grid-cols-1">
          {gallery.map((item, index) => {
            ...
            return (
              <div
                key={item.id || index}
                className={`${colSpanClass} min-h-[600px] lg:min-h-0 3xl:min-h-[800px]`}
              >
                <PrimaryImage
                  url={item.src}
                  size="tablet"
                  alt={item.alt || `tent-${index}`}
        ...
        <div className="w-full flex justify-center items-center">
          <DownloadButton
            link={masterClassTentingURL}
            text="DOWNLOAD MASTERCLASS TENTING 101"
            classes={"lg:!w-[656px] sm:!w-[492px] 3xl:!w-[1000px]"}
            iconTrue={"true"}
          />
        </div>
      </div>
